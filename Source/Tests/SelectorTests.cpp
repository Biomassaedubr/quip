...
TEST_CASE("One can select the next word from a word.", "[SelectorTests]") {
  Document document("Hello world!");
...
TEST_CASE("One can select the next word from whitespace.", "[SelectorTests]") {
  Document document("Hello world!");
...
TEST_CASE("One can select the next word across a newline.", "[SelectorTests]") {
  Document document("Hello world \nand friends!");
  Optional<Selection> result = selectNextWord(document, Selection(Location(6, 0)));
  
  REQUIRE(result->origin() == Location(0, 1));
  REQUIRE(result->extent() == Location(3, 1));
}
